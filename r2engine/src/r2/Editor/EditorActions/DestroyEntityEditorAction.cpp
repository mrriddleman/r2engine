...
namespace r2::edit
{
		...
		,mParentOfEntityToDestory(parentOfEntityToDestroy)
	{
		...
		mEntityEditorNameComponent = mnoptrEditor->GetECSCoordinator()->GetComponent<ecs::EditorNameComponent>(mEntityToDestroy);
	...
	void DestroyEntityEditorAction::Undo()
	{
		...
		coordinator->AddComponent<ecs::EditorNameComponent>(mEntityToDestroy, mEntityEditorNameComponent);
	...
	{
		//mnoptrEditor->GetSceneGraph().DetachChildren(mEntityToDestroy);
