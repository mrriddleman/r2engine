...
float SampleSpotlightShadowMapDepth(vec2 base_uv, float u, float v, vec2 shadowMapSizeInv, int64_t spotlightID)
{
	vec2 uv = base_uv + vec2(u, v) * shadowMapSizeInv;
...
float SampleSpotlightShadowMap(vec2 base_uv, float u, float v, vec2 shadowMapSizeInv, int64_t spotlightID, float depth)
{
	float shadowSample = SampleSpotlightShadowMapDepth(base_uv, u, v, shadowMapSizeInv, spotlightID);
	return depth > shadowSample ? 1.0 : 0.0;
}
...
float OptimizedPCF7(vec3 shadowPosition, uint cascadeIndex, int64_t lightID, float lightDepth)
{
    ...
    sum += uw2 * vw0 * SampleDirectionShadowMap(base_uv, u2, v0, shadowMapSizeInv, cascadeIndex, lightID, lightDepth);
    sum += uw3 * vw0 * SampleDirectionShadowMap(base_uv, u3, v0, shadowMapSizeInv, cascadeIndex, lightID, lightDepth);

    sum += uw0 * vw1 * SampleDirectionShadowMap(base_uv, u0, v1, shadowMapSizeInv, cascadeIndex, lightID, lightDepth);
    sum += uw1 * vw1 * SampleDirectionShadowMap(base_uv, u1, v1, shadowMapSizeInv, cascadeIndex, lightID, lightDepth);
    sum += uw2 * vw1 * SampleDirectionShadowMap(base_uv, u2, v1, shadowMapSizeInv, cascadeIndex, lightID, lightDepth);
    sum += uw3 * vw1 * SampleDirectionShadowMap(base_uv, u3, v1, shadowMapSizeInv, cascadeIndex, lightID, lightDepth);

    sum += uw0 * vw2 * SampleDirectionShadowMap(base_uv, u0, v2, shadowMapSizeInv, cascadeIndex, lightID, lightDepth);
    sum += uw1 * vw2 * SampleDirectionShadowMap(base_uv, u1, v2, shadowMapSizeInv, cascadeIndex, lightID, lightDepth);
    sum += uw2 * vw2 * SampleDirectionShadowMap(base_uv, u2, v2, shadowMapSizeInv, cascadeIndex, lightID, lightDepth);
    sum += uw3 * vw2 * SampleDirectionShadowMap(base_uv, u3, v2, shadowMapSizeInv, cascadeIndex, lightID, lightDepth);

    sum += uw0 * vw3 * SampleDirectionShadowMap(base_uv, u0, v3, shadowMapSizeInv, cascadeIndex, lightID, lightDepth);
    sum += uw1 * vw3 * SampleDirectionShadowMap(base_uv, u1, v3, shadowMapSizeInv, cascadeIndex, lightID, lightDepth);
    sum += uw2 * vw3 * SampleDirectionShadowMap(base_uv, u2, v3, shadowMapSizeInv, cascadeIndex, lightID, lightDepth);
    sum += uw3 * vw3 * SampleDirectionShadowMap(base_uv, u3, v3, shadowMapSizeInv, cascadeIndex, lightID, lightDepth);

    return sum * (1.0f / 2704.0f);
}

float OptimizedPCF5(vec3 shadowPosition, uint cascadeIndex, int64_t lightID, float lightDepth)
{
	vec2 uv = shadowPosition.xy * shadowMapSizes[cascadeIndex];
